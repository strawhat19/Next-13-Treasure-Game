...
export default function Profile() {
  const { updates, setUpdates, user, setPage, highScore, colorPref, setColorPref } = useContext(StateContext);
      ...
      <div className="inner">
        {user ? <h1>{user?.name}</h1> : <h2><i>Signed Out</i></h2>}
          ...
          <div className={`column rightColumn gameStats`}>
            <button title="Click to View High Scores" style={{background: `var(--blackGlass)`, borderRadius: 4, justifyContent: `center`, alignItems: `center`, maxWidth: `fit-content`, padding: `5px 15px`, pointerEvents: `none`}} className={`flex row`}><h2 className={`flex row`}><i style={{color: `var(--gameBlue)`}} className="fas fa-signal"></i><span className="highScore">{Math.floor(highScore).toLocaleString(`en-US`)}</span><span className="label">High Score</span></h2></button>
            <button onClick={() => setColorPref(!colorPref)} title="Click to View High Scores" style={{background: `var(--blackGlass)`, borderRadius: 4, justifyContent: `center`, alignItems: `center`, maxWidth: `fit-content`, padding: `5px 15px`, marginTop: 10}} className={`flex row`}><h2 className={`flex row`}><span className="label">{colorPref ? `Default Color` : `Change Color`}</span><i style={{color: `var(--gameBlue)`}} className="fas fa-question"></i></h2></button>
          </div>
      </div>
    </section>
